...
public class DaoUsuario extends DaoGenerico<Usuario, Integer> {
	...
	// Filtrat por nombre y apellido y mostrar los id
	public List<Integer> filtrar_nombre_apell(String nombre, String ape1, String ape2) throws BusinessException{
		...
		try {
			String sql = " select idusuario from usuario where"
					+ " nombre=? and apellido1=? and apellido2=?";
			pstm = con.prepareStatement(sql, ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
	...
	// Filtrat por nombre y apellido y mostrar los id
		public List<Integer> filtrar_nombre_apell2(String nombre, String ape1, String ape2) throws BusinessException{
			List<Integer> result = new ArrayList<Integer>();
			Connection con = ConexionJdbc.getConnection();
			PreparedStatement pstm = null;
			ResultSet rs = null;
			try {
				...
				String sql = " select idusuario from usuario where 1";
				if(!nombre.equals("")) sql+=" and nombre=?";
				if(!ape1.equals("")) sql+=" and apellido1=? ";
				if(!ape2.equals("")) sql+=" and apellido2=? ";
				...
				if(!nombre.equals(""))pstm.setString(numparam++, nombre);
				if(!ape1.equals(""))pstm.setString(numparam++, ape1);
				if(!ape2.equals(""))pstm.setString(numparam++, ape2);
		...
		public List<Usuario> buscarPorDepartamento(Integer id) throws BusinessException {
			...
			ResultSet rs = null;
			try {
				String sql = "SELECT * FROM usuario WHERE iddepartamento=?";
				pstm = con.prepareStatement(sql, ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
				pstm.setInt(1, id);
				rs = pstm.executeQuery();
				if (rs.first()) {
